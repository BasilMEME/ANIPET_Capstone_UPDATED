    ...
    AppContainer {
        ...
        Column(modifier = contentModifier, verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                StandardCard(title = "Applicant's Info") {
                    ...
                    Spacer(modifier = Modifier.height(8.dp))
                    if (interactionMethod == "Phone") {
                        OutlinedTextField(
                            value = phone,
                            onValueChange = { phone = it },
                            label = { Text("Phone Number for Call Verification*") },
                            modifier = Modifier.fillMaxWidth()
                        )
                        Spacer(modifier = Modifier.height(8.dp))
                    }
                    ...
                    onClick = {
                        ...
                        scope.launch {
                            try {
                                ...
                                if (address.isBlank() || email.isBlank() || birthDate.isBlank() || company.isBlank() || status.isBlank() || pronouns.isBlank() || promptedBy.isBlank() || adoptedBefore.isBlank() || interactionMethod.isBlank() || lookingFor.isBlank() || specificAnimal.isBlank() || idealPet.isBlank() || buildingType.isBlank() || doRent.isBlank() || movePlan.isBlank() || household.isBlank() || allergic.isBlank() || dailyCaregiver.isBlank() || financialResponsible.isBlank() || petSitter.isBlank() || hoursLeft.isBlank() || familySupport.isBlank() || otherPets.isBlank() || pastPets.isBlank() || willVisit.isBlank()) {
                                    statusText = "Please complete all required fields before submitting."
                                    return@launch
                                }
                                if (interactionMethod == "Phone" && phone.isBlank()) {
                                    statusText = "Please provide a phone number for Phone Call verification."
                                    return@launch
                                }
                                if (interactionMethod == "Zoom") {
