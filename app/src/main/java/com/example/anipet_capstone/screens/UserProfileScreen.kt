    ...
    AppContainer() {
        ...
        if (profile?.user != null) {
            StandardCard(title = "Account Details") {
                InfoText("Full Name", profile!!.user!!.full_name)
                InfoText("Username", profile!!.user!!.username ?: "-")
                InfoText("Email", profile!!.user!!.email)
                InfoText("Role", profile!!.user!!.role ?: "user")
                InfoText("Verified", if (profile!!.user!!.role == "user") "Yes" else "No")
            }
        } else {
            StandardCard {
